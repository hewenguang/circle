...
export default function (root: HTMLElement) {
  ...
  each(
    // @ts-ignore
    parent ? [...parent.children, ...root.children] : root.children,
    (item: Element) => {
      if (!item || !isTag(item, 'style')) {
        return;
      }
      const stylesheet = item.textContent || '';
      stylesheet.length > 0 && (style += stylesheet);
    }
  );
  ...
  if (attrs.length <= 0) {
    return [template, style].join('').replace(/\n+/g, '');
  }
  each(attrs, (attr: string) => {
    const [key, value] = attr.split(':');
    style = style.replace(new RegExp(`var\\(${key}\\)`, 'g'), value);
  });
  return [template, style].join('').replace(/\n+/g, '');
}
